...
function createRoomStore() {
	...
	// const joinRoom = update((room) => {
	// 	console.log('Join room: ', room.roomId);
	// 	if (!room.roomId) {
	// 		return;
	// 	}
	// 	const roomCollection = collection(firestore, 'rooms');
	// 	const roomDoc = doc(roomCollection, room.roomId);
	// 	const roomSnapshot =  getDoc(roomDoc);
	// 	console.log('Got room:', roomSnapshot.exists);

	// 	if (roomSnapshot.exists()) {
	// 		console.log('Create PeerConnection with configuration: ', configuration);
	// 		room.peerConnection = new RTCPeerConnection(configuration);
	...
	// 	}
	// 	console.log('Got offer:', offer);
	// 	await room.peerConnection.setRemoteDescription(new RTCSessionDescription(offer));
	...
	// 	// Listening for remote ICE candidates below
	// 	// eslint-disable-next-line @typescript-eslint/no-unused-vars
	// 	const unsub = onSnapshot(collection(roomDoc, 'callerCandidates'), (snapshot) => {
	// 		snapshot.docChanges().forEach(async (change) => {
	// 			if (change.type === 'added') {
	// 				const data = change.doc.data();
	// 				console.log(`Got new remote ICE candidate: ${JSON.stringify(data)}`);
